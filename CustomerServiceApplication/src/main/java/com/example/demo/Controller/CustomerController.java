...
public class CustomerController {
    ...
    public ResponseEntity<CustomerAccount> createCustomer( @RequestBody CustomerAccount customer1) {
        try {
            Customer customer = customerRepo.save(new Customer(customer1.getCustomer().getCustomerName(),customer1.getCustomer().getAge(),customer1.getCustomer().getCity(),customer1.getCustomer().getEmailid()));
            ...
            restTemplate.exchange("http://AccountServiceApplication/account/createAccount",HttpMethod.POST,httpEntity,void.class);
